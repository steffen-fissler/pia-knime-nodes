...
public class WizardPanelPSM extends JPanel implements ActionListener {
	...
	private PIAAnalysisModel piaViewModel;
	...
	public WizardPanelPSM(PIAAnalysisModel piaViewModel) {
	...
	public Map<String, Object> getSettings() {
		...
		settings.put(PIASettings.ALL_DECOY_STRATEGY.getKey(), decoyStrategyGroup.getSelection().getActionCommand());
		settings.put(PIASettings.ALL_DECOY_PATTERN.getKey(), decoyPattern_pattern.getText());
		settings.put(PIASettings.ALL_USED_IDENTIFICATIONS.getKey(), Integer.parseInt(usedIdentificationsGroup.getSelection().getActionCommand()));
	 ...
	 * @param modeller
	 */
	public void setPIAViewModel(PIAAnalysisModel model) {
	...
	public void applyLoadedSettings(NodeSettingsRO settings) {
		...
		if (settings.getString(PIASettings.ALL_DECOY_STRATEGY.getKey(), PIASettings.ALL_DECOY_STRATEGY.getDefaultString()).
		...
		if (settings.getInt(PIASettings.ALL_USED_IDENTIFICATIONS.getKey(), PIASettings.ALL_USED_IDENTIFICATIONS.getDefaultInteger()) == 0) {
	...
	private class ScoreNameListCellRenderer extends JLabel implements ListCellRenderer<ScoreModel> {
	    ...
	    // TODO: deprecate this class and use ScoreCellRenderer instead
